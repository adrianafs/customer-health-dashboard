...
function AiSidebar({
  ...
  onToggle: () => void
  activeCsmId: string
  csmList: { name: CSMName; ownerId: string }[]
}) {
  ...
  function buildSystem() {
    const sum = clients
      .map(c =>
        `- ${c.name} (CSM: ${c.csm}): ${STATE_LABELS[c.healthState]}, score ${c.score}/100, ${c.lastContactDaysAgo}d since last contact, €${formatARR(c.arr)}. ${c.whyThisScore}`
      )
      .join('\n')

    const totalARR = clients.reduce((s, c) => s + c.arr, 0)
    const riskARR = clients
      .filter(c => ['action_required', 'churn_risk'].includes(c.healthState))
      .reduce((s, c) => s + c.arr, 0)

    // Resolve active CSM name
    const activeCsmName =
      activeCsmId === 'all'
        ? null
        : csmList.find(c => c.ownerId === activeCsmId)?.name ?? activeCsmId

    const viewContext = activeCsmName
      ? `The CSM currently logged in is ${activeCsmName}. You are acting as their personal assistant. Only refer to their accounts — do not mention other CSMs' accounts unless directly asked.`
      : `You are viewing all CSMs combined. Mention the responsible CSM when discussing specific accounts.`

    return `You are Claude, embedded in Flowbox's internal Customer Success dashboard. You help CSMs manage their B2B SaaS accounts.

${viewContext}

Today: ${new Date().toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}.
Accounts in current view: ${clients.length} · €${formatARR(totalARR)} total ARR · €${formatARR(riskARR)} at risk.

ACCOUNTS IN CURRENT VIEW:
${sum}

Be concise, direct and actionable. Use bullet points. Reference specific account data. If the CSM asks "my accounts" or "my clients", refer only to the accounts listed above.`
  }
  ...
  async function send() {
    ...
    try {
      ...
      setMsgs(prev => [
        ...prev,
        { role: 'assistant', text: data.text || 'Could not get a response.', ts: now() },
      ])
    } catch {
      setMsgs(prev => [
        ...prev,
        { role: 'assistant', text: 'Unable to reach Claude. Check your ANTHROPIC_API_KEY.', ts: now() },
      ])
    } finally {
      setLoading(false)
    }
  ...
  // Reset conversation when CSM filter changes so context is always fresh
  useEffect(() => {
    setMsgs([])
  }, [activeCsmId])
  ...
  // Dynamic chips based on whether a specific CSM is selected
  const activeCsmName =
    activeCsmId === 'all'
      ? null
      : csmList.find(c => c.ownerId === activeCsmId)?.name ?? null
  ...
  const CHIPS = activeCsmName
    ? [
        ...
        { label: 'What should I do today?', q: `Give me a prioritised action list for today` },
      ]
    : [
  ...
  const welcomeText = activeCsmName
    ? `Hi ${activeCsmName.split(' ')[0]}! I'm watching your ${clients.length} accounts. Ask me anything about your portfolio.`
    : `Watching ${clients.length} accounts across all CSMs. Ask me anything about churn risk, renewals, or priorities.`
      ...
      >
        ...
        >
          <svg width="13" height="13" viewBox="0 0 12 12" fill="none">
            <path d="M6 .75l1.5 3.5 3.5 1.5-3.5 1.5L6 10.75 4.5 7.25 1 5.75l3.5-1.5L6 .75z" fill="white" />
          </svg>
        </div>
        {open && (
          <>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--n500)', whiteSpace: 'nowrap', marginTop: 1 }}>
                {activeCsmName ? `${activeCsmName.split(' ')[0]}'s assistant` : 'CS intelligence'}
              </div>
            </div>
            <button
              onClick={onToggle}
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M9 3l4 4-4 4M1 7h12" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </>
        )}
        {!open && (
          ...
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M5 3l-4 4 4 4M13 7H1" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        )}
      </div>
        ...
        <>
          ...
          >
            ...
            {msgs.length === 0 && (
              <div
                style={{
                  background: 'linear-gradient(135deg,var(--v500) 0%,var(--v300) 100%)',
                  borderRadius: 16,
                  padding: 18,
                  color: '#fff',
                  position: 'relative',
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, letterSpacing: '-.01em' }}>
                    {activeCsmName ? `Hi ${activeCsmName.split(' ')[0]}!` : "Hi, I'm Claude"}
                  </div>
                  <div style={{ fontSize: 11.5, opacity: 0.82, lineHeight: 1.55, marginTop: 4 }}>
                    {welcomeText}
                  </div>
              ...
              >
                  ...
                  >
                    ...
                    >
                      <svg width="9" height="9" viewBox="0 0 9 9" fill="none">
                        <path
                          d="M4.5.75l1.2 2.8 2.8 1.2-2.8 1.2-1.2 2.8-1.2-2.8L.75 4.75l2.55-1.2L4.5.75z"
                          fill="white"
                        />
                      </svg>
                    </div>
                ...
                <div
                  style={{
                    ...
                    fontSize: 12.5,
                    lineHeight: 1.65,
                    background: m.role === 'user' ? 'var(--v500)' : 'var(--n50)',
                    color: m.role === 'user' ? '#fff' : 'var(--n900)',
                    border: m.role === 'assistant' ? '1px solid var(--n200)' : 'none',
                  }}
                  dangerouslySetInnerHTML={{
                    __html: m.text
                      ...
                      .replace(
                        /^- (.+)$/gm,
                        '<div style="display:flex;gap:5px;margin:2px 0"><span style="color:var(--v500);font-weight:700;flex-shrink:0">·</span><span>$1</span></div>'
                      )
                      .replace(/\n\n/g, '<br><br>')
                      .replace(/\n/g, '<br>'),
                  }}
                />
                {m.role === 'user' && (
                  <span style={{ fontSize: 10, color: 'var(--n400)', padding: '0 3px' }}>{m.ts}</span>
                )}
              </div>
            ))}
                  ...
                  {[0, 1, 2].map(i => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: '50%',
                        background: 'var(--n400)',
                        animation: `pulse .75s ease-in-out ${i * 0.2}s infinite`,
                      }}
                    />
                  ))}
                </div>
          ...
          <div style={{ padding: '12px 14px', borderTop: '1px solid var(--n100)', flexShrink: 0 }}>
            ...
            >
              <textarea
                ...
                placeholder={
                  activeCsmName
                    ? `Ask about your accounts, ${activeCsmName.split(' ')[0]}…`
                    : 'Ask Claude about your accounts…'
                }
                rows={1}
                style={{
                  flex: 1,
              ...
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <path d="M1 1.5l12 5.5L1 12.5V8.5L9 7 1 5.5V1.5z" fill="white" />
                </svg>
              </button>
            </div>
          </div>
...
export default function Dashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setApiError('0 customers returned')
      }
    } catch (e) {
  ...
  const counts = useMemo(() => {
    const c: Record<HealthState, number> = {
      stable: 0,
      keep_an_eye: 0,
      action_required: 0,
      churn_risk: 0,
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: 'var(--font)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div
                ...
                style={{
                  ...
                  borderTopColor: 'var(--v500)',
                }}
              />
              <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--n900)', letterSpacing: '-.02em' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--n900)', letterSpacing: '-.025em', lineHeight: 1 }}>
                Customer Health
              </div>
              <div style={{ fontSize: 10, fontWeight: 500, color: 'var(--n500)', letterSpacing: '.05em', textTransform: 'uppercase', marginTop: 1 }}>
                Flowbox CS · Internal
              </div>
            </div>
          </div>
          ...
          <div
            style={{
              ...
              background: dataSource === 'hubspot' ? 'var(--s50)' : 'var(--v50)',
              border: `1px solid ${dataSource === 'hubspot' ? '#00CC9A' : 'var(--v200)'}`,
              color: dataSource === 'hubspot' ? 'var(--s600)' : 'var(--v600)',
            }}
          >
            <div
              style={{
                ...
                height: 6,
                borderRadius: '50%',
                background: dataSource === 'hubspot' ? '#00CC9A' : 'var(--v500)',
              }}
          ...
          >
            {loading ? (
              ...
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d="M10.5 6A4.5 4.5 0 111.5 6M10.5 2.5V6H7" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            )}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
            ...
            {apiError && (
              <div
                style={{
                  padding: '6px 24px',
                  background: 'var(--d50)',
                  borderBottom: '1px solid var(--d500)',
                  fontSize: 11,
                  color: 'var(--d600)',
                  display: 'flex',
                  gap: 8,
              ...
              {COLUMNS.map((col, i) => (
                <div
                  ...
                  style={{
                    ...
                    alignItems: 'center',
                    gap: 12,
                    borderLeft: i > 0 ? '1px solid var(--n100)' : 'none',
                  }}
                >
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 8,
                      background: STAT_ICONS[col.state].bg,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                  ...
                  <div>
                    <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-.03em', lineHeight: 1, color: STATE_COLORS[col.state] }}>
                      {counts[col.state]}
                    </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 24px' }}>
              ...
              >
                {[{ name: 'All CSMs', ownerId: 'all' }, ...csmList].map(csm => (
                  <button
                    key={csm.ownerId}
                    onClick={() => { setActiveCsmId(csm.ownerId); setSelectedClient(null) }}
                    style={{
                      padding: '5px 13px',
                      borderRadius: 6,
                      border: 'none',
                      ...
                      cursor: 'pointer',
                      transition: 'all .12s',
                      background: activeCsmId === csm.ownerId ? 'var(--n0)' : 'none',
                      color: activeCsmId === csm.ownerId ? 'var(--n900)' : 'var(--n500)',
                      boxShadow: activeCsmId === csm.ownerId ? 'var(--ss)' : 'none',
                    }}
                  >
                    {csm.name}
                  </button>
              ...
              <div style={{ position: 'relative', flex: 1, maxWidth: 220 }}>
                ...
                >
                  ...
                  <path d="M9.5 9.5L11.5 11.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
                </svg>
                <input
                  value={searchQuery}
              ...
              <span style={{ fontSize: 11, color: 'var(--n500)', marginLeft: 'auto' }}>
                ...
                <div key={col.state}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--n500)', background: 'var(--n100)', borderRadius: 999, padding: '1px 7px' }}>
                        {colClients.length}
                      </span>
                    </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {colClients.length === 0 ? (
                      <div style={{ padding: '28px 16px', borderRadius: 12, border: '1.5px dashed var(--n200)', fontSize: 12, color: 'var(--n400)', textAlign: 'center' }}>
                        No accounts
                      </div>
                    ) : (
                      colClients.map((c, i) => (
        ...
        {/* AI Sidebar — now CSM-aware */}
        <AiSidebar
          clients={clients}
          open={aiOpen}
          onToggle={() => setAiOpen(v => !v)}
          activeCsmId={activeCsmId}
          csmList={csmList}
        />
      </div>
